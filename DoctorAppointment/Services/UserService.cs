...
namespace DoctorAppointment.Services
{
    public class UserService:IUserService
    {
        ...
        public async Task<GetUserAppointmentsResponse> GetUserAppointmentsAsync(string userId)
        {
            ...
            if (user == null)
            {
                return new GetUserAppointmentsResponse { Success = false, Message = "User not found." };
            }

            var appointments = await _userRepository.GetUserAppointmentsAsync(userId);
            if (appointments == null || !appointments.Any())
            {
                return new GetUserAppointmentsResponse { Success = false, Message = "No appointments found." };
            }

            return new GetUserAppointmentsResponse
            {
                Success = true,
        ...
        public async Task<CancelAppointmentResponse> CancelAppointmentAsync(string userId, string appointmentId)
        {
            ...
            if (doctor == null || !doctor.Available)
            {
                return new CancelAppointmentResponse { Success = false, Message = "Doctor not available." };
            }

            if (doctor.SlotsBooked.ContainsKey(appointment.SlotDate))
            {
                doctor.SlotsBooked[appointment.SlotDate].Remove(appointment.SlotTime);
                if (doctor.SlotsBooked[appointment.SlotDate].Count == 0)
                {
                    doctor.SlotsBooked.Remove(appointment.SlotDate);
                }
            }
